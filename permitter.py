...
-----END PGP PUBLIC KEY BLOCK-----
    ...
    def exportHTML(self):
        try:
            ...
            if result == JFileChooser.APPROVE_OPTION:
                ...
                with self.test_lock:
                    results_snapshot = list(self.test_results)
                def _do_export_html(path, results):
                    try:
                        with open(path, 'w') as f:
    ...
    def _sanitizeForReport(self, b):
        if b is None:
            return ""
        s = self.helpers.bytesToString(b)
        truncated = len(s) > self.MAX_EMBED
        if truncated:
            s = s[:self.MAX_EMBED]
    ...
    def _writeHTMLReport(self, f, results):
        try:
            timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
            status_counts = {}
            role_counts = {}
    ...
    """
            ...
            for i, result in enumerate(results):
                ...
                try:
                    request_content = self._sanitizeForReport(self._resultRequest(result))
                except Exception:
                    request_content = "Error displaying request"
                try:
                    response_content = self._sanitizeForReport(self._resultResponse(result))
                except Exception:
